...
\begin{tabular}{c l c}
		$V'$ & Volumen nach Ausdehnung & $[A'] = \mathrm{m^3}$ \\
		$V$ & Anfangsvolumen & $[A] = \mathrm{m^3}$ \\
		$\Delta \, V$ & Volumenänderung & $[\Delta \, V] = \mathrm{m^3}$ \\
		\rule{0pt}{8pt}$\gamma$ & Volumenausdehnungskoeffizient & $[\beta] = \mathrm{\frac{1}{K}}$ \\ 
		$\Delta \, T $ & Temperaturänderung & $[\Delta \, T ] = \mathrm{K}$ \\
\end{tabular}
...
$$ T > \Theta_D: \quad C_m \approx 3 \, R \approx 25 \frac{J}{mol}  \qquad \mathrm{(Dulung-Petit)}$$
...
%$$ \boxed{ f_r = \frac{p_D}{p_S} = \frac{p_D}{p_S(\theta)} } $$
